...
class RunExecutionFenceServiceTest {
	...
	@Test
	void lockedFenceRejectsExpiredRunBeforeSideEffect() {
		QueryRunService runs = mock(QueryRunService.class);
		when(runs.lockForUpdate("run-1")).thenReturn(QueryRun.builder()
			.runId("run-1")
		...
		assertThatThrownBy(() -> new RunExecutionFenceService(runs).assertActiveAndLock("run-1", "attempt-1"))
			.isInstanceOf(RunDeadlineExceededException.class);
	}

	@Test
	void finalizerFenceOnlyAllowsTheBoundTerminalAttempt() {
		QueryRunService runs = mock(QueryRunService.class);
		when(runs.lockForUpdate("run-1")).thenReturn(run(RunStatus.SUCCEEDED, "attempt-1", "instance-a"));
		RunExecutionFenceService fence = new RunExecutionFenceService(runs);

		assertThatCode(() -> fence.assertFinalizerOwnsRunAndLock("run-1", "attempt-1")).doesNotThrowAnyException();
		assertThatThrownBy(() -> fence.assertFinalizerOwnsRunAndLock("run-1", "attempt-2"))
			.isInstanceOf(LateRunResultDroppedException.class);
	}
